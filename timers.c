...
#define ADV_UPDATE_INTERVAL                 APP_TIMER_TICKS(10*1000)          /* 10 sec */
...
/* Advertising update timer stop */
void advertising_update_timer_stop()
{
  ret_code_t err_code;
  err_code = app_timer_stop(m_update_adv_timer_id);
  APP_ERROR_CHECK(err_code);
}
...
static void update_sensor_characteristic_timer_start() {
  ...
  /* Start Advertising data update timer */
...
static void update_sensor_characteristic_timer_stop()
{
  ...
  /* Stop Advertising data update timer */
  advertising_update_timer_stop();
